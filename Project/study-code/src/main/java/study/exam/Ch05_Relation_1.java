...
public class Ch05_Relation_1 {
    ...
    private static void deleteRelation(EntityManager manager) {
        Member member = manager.find(Member.class, "member");
        member.setTeam1(null);
    }
    ...
    private static void updateRelation(EntityManager manager) {
        Team team12 = new Team("team2", "팀2");
        manager.persist(team12);

        Member member = manager.find(Member.class, "member");
        member.setTeam1(team12);
    }
    ...
    private static void testSave(EntityManager manager) {
        //팀 저장
        Team team = new Team("team", "개발팀");
        manager.persist(team); //영속성 처리

        //이요한 회원 저장.
        Member member = new Member("member", "이요한");
        member.setTeam1(team);
        manager.persist(member);

        Member member12 = new Member("member12", "홍길동");
